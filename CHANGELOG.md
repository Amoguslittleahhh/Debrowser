...
Verification: 57 checks, all green, including these — that the updater is inert outside a packaged build and names the reason. Packing was checked rather than assumed: this project's `files:` allow-list does not mention `node_modules`, so the new dependency could plausibly have been left out of the archive and crashed only on an installed copy. It is packed, and `app-update.yml` lands beside it.
